...
function signInUser()
{
    ...
    if (isset($assocResult[0]) && password_verify($password, $assocResult[0]["password_user"])) {
        ...
        if (isset($_POST['rememberme']) && $_POST['rememberme'] == 'on') {
            setcookie("id_user", $assocResult['id_user'], time() + 86400);
            setcookie("cookie_hash", $assocResult['user_password'], time() + 86400);
        }
...
function checkAuthWithCookie(){
    ...
    if(isset($_COOKIE['id_user']) && isset($_COOKIE['cookie_hash'])){
        $db = getConnection();
        $sql = "select id_user, name_user, password_user from user where id_user = '".mysqli_real_escape_string($db, strip_tags($_COOKIE['id_user']))."'";
        $assocResult = getAssocResult($sql);
        mysqli_close($db);
